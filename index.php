...
<body class="d-flex flex-column h-100">
    <main class="flex-shrink-0">
        ...
        <header class="bg-dark py-5">
            ...
            //This query is  for selecting the homepage image from the about table of database
            $query = "SELECT AboutImage FROM about";

            //Passes the query in the database through the connection
            $result = $conn->query($query);

            //Recieving the result of the query and storing it as an associative array
            $row = $result->fetch_assoc();

            //Getting and storing the image 
            $image = $row["AboutImage"];

            //This query returns the text for the homepage and does the same like previous query
            $query2 = "SELECT HomePage FROM about";

            $result2 = $conn->query($query2);
            $row2 = $result2->fetch_assoc();
            $text = $row2["HomePage"];

            //Introducing and printing a heredoc to show the result
            $show = <<<HTML
                <div class="container px-5">
